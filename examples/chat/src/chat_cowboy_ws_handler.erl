...
websocket_init(_Type, Req, _Opts) ->
    ...
    Handler =
	ebus_proc:spawn_handler(fun chat_erlbus_handler:handle_msg/2,
				[self()]),
    Msg = erlang:iolist_to_binary([get_name(Req),
				   <<" has entered the chat.">>]),
    ebus:pub(?CHATROOM_NAME, {<<"System">>, Msg}),
    ebus:sub(Handler, ?CHATROOM_NAME),
    {ok, Req,
     #state{name = get_name(Req), handler = Handler},
     ?TIMEOUT}.
...
websocket_info({message_published, {Sender, Msg}}, Req,
	       State) ->
    {reply,
     {text, jiffy:encode({[{sender, Sender}, {msg, Msg}]})},
     Req, State};
websocket_info(_Info, Req, State) -> {ok, Req, State}.

websocket_terminate(_Reason, _Req, State) ->
    {_, From, _} = State,
    Msg = erlang:iolist_to_binary([From,
				   <<" has left the chat.">>]),
...
get_name(Req) ->
    ...
    Name =
	list_to_binary(string:join([inet_parse:ntoa(Host), ":",
